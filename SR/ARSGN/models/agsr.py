...
""" class Upsampler(nn.Sequential):
    # Esempio di implementazione di un blocco di Upsampling per fattore x4
    def __init__(self, conv, scale, n_feat, bn=False, act=False, bias=True):
        m = []
        if (scale & (scale - 1)) == 0:    # Potenze di 2 (2, 4, 8, ...)
            for _ in range(int(math.log(scale, 2))):
                m.append(conv(n_feat, 4 * n_feat, 3, bias))
                m.append(nn.PixelShuffle(2)) # <-- ESEGUE L'INGRANDIMENTO x2
                if bn: m.append(nn.BatchNorm2d(n_feat))
                if act: m.append(act())
        # ... (gestione di altri fattori di scala se necessario)
